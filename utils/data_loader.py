...
Carga y procesamiento central de datos para Grupo Jaloma PoC
"""
...
def _detectar_churn(ventas: pd.DataFrame, dias_umbral: int = 18) -> list:
    """Clientes que llevan más de N días sin comprar (señal de churn)"""
...
def get_forecast_data():
    ...
    # Forecast para SKUs estrella de Jaloma
    skus_forecast = ["SKU014", "SKU041", "SKU020", "SKU001", "SKU031", "SKU039"]
    resultados = []
...
def get_alertas():
    ...
    # SKUs con menos de 14 días de inventario y alta rotación
    ...
    riesgo_quiebre.loc[:, "venta_en_riesgo"] = (
        riesgo_quiebre["demanda_semana"] * 2 * riesgo_quiebre["precio_lista_mxn"] * 0.92
    ).round(0)
    ...
    # ── ALERTA 3: Clientes en riesgo de churn ───────────────────────────
