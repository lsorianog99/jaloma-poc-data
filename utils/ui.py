"""
utils/ui.py
Componentes visuales reutilizables para Grupo Jaloma PoC
"""
...
def inject_css():
    ...
    /* Header de página */
    .page-header {{
        background: linear-gradient(135deg, {JALOMA_COLORS["primary"]} 0%, #8B0000 100%);
        padding: 2rem 2.5rem;
        border-radius: 16px;
        color: white;
    ...
    .kpi-card {{
        ...
        border-left: 4px solid {JALOMA_COLORS["primary"]};
        box-shadow: 0 1px 8px rgba(0,0,0,0.06);
    }}
    .kpi-label  {{ font-size: 0.78rem; color: {JALOMA_COLORS["muted"]}; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }}
    .kpi-value  {{ font-size: 1.9rem; font-weight: 700; color: {JALOMA_COLORS["dark"]}; line-height: 1.1; }}
    .kpi-delta  {{ font-size: 0.82rem; color: {JALOMA_COLORS["muted"]}; margin-top: 0.2rem; }}
    .kpi-danger {{ border-left-color: {JALOMA_COLORS["danger"]}; }}
    .kpi-warn   {{ border-left-color: {JALOMA_COLORS["warning"]}; }}
    .kpi-success{{ border-left-color: {JALOMA_COLORS["success"]}; }}

    /* Alerta Cards */
    .alerta-critica {{
        background: white;
        border: 1.5px solid {JALOMA_COLORS["danger"]};
        border-left: 6px solid {JALOMA_COLORS["danger"]};
        border-radius: 12px;
    ...
    .alerta-warning {{
        background: white;
        border: 1.5px solid {JALOMA_COLORS["warning"]};
        border-left: 6px solid {JALOMA_COLORS["warning"]};
        border-radius: 12px;
        padding: 1.4rem 1.6rem;
        margin-bottom: 1rem;
    ...
    .alerta-info {{
        background: white;
        border: 1.5px solid {JALOMA_COLORS["primary"]};
        border-left: 6px solid {JALOMA_COLORS["primary"]};
        border-radius: 12px;
        padding: 1.4rem 1.6rem;
        margin-bottom: 1rem;
        box-shadow: 0 2px 12px rgba(200,16,46,0.10);
    ...
    .badge-red   {{ background: #FEE2E2; color: {JALOMA_COLORS["primary"]}; }}
    .badge-orange{{ background: #FEF3C7; color: #92400E; }}
    .badge-green {{ background: #D1FAE5; color: #065F46; }}

    .alerta-titulo {{ font-size: 1.05rem; font-weight: 700; color: {JALOMA_COLORS["dark"]}; margin-bottom: 0.4rem; }}
    ...
    .stChatInput input {{
        ...
        border: 1.5px solid {JALOMA_COLORS["primary"]} !important;
    }}
